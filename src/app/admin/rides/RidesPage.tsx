...
export function RidesPage() {
  const { data, isLoading } = useApiQuery<{ data: Ride[]; meta: any }>(
    ['rides'],
    '/rides'
  );
  ...
  const columns: ColumnDef<Ride>[] = [
    {
      ...
      cell: ({ row }) => {
        ...
        try {
          ...
          return <span className="text-sm text-slate-500">Data Inválida</span>;
        }
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <div className="font-medium">
            {row.original.driver?.user?.name || 'N/A'}
          </div>
          <div className="text-xs text-slate-500">
            {row.original.driver?.licensePlate}
          </div>
        </div>
      ),
    },
    {
      ...
      header: 'Valor',
      cell: ({ row }) => (
        <span className="font-mono">{row.getValue('amount')} AKZ</span>
      ),
    },
    {
      accessorKey: 'status',
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Acompanhe o histórico de viagens em tempo real.
          </p>
        </div>
      </div>
